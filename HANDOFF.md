...
**Date:** 2026-05-16
**Branch:** `main` (опережает `origin/main` на 18 коммитов; рабочее дерево чистое кроме untracked `backups/`) — последний коммит `3ff636f` fix(deploy/nginx): quote regex with brace-quantifier so nginx parses it (2026-05-16).
...
- **Найдены два nginx-нюанса в `deploy/nginx/alexanderlapygin.com.conf`:**
  - **regex-footgun:** `location ~* /assets/.+-[A-Za-z0-9_-]{8,}\.(?:...)$` — nginx тащит `{` как начало блока, ломая `nginx -t`. Починено обёрткой regex в двойные кавычки, закоммичено как `3ff636f`. Поймал реальный cutover, потому что staging-vhost не воспроизводил `/showcase/` location.
...
- `5fb91e2` docs(handoff): update for session 2026-05-16
- `3ff636f` fix(deploy/nginx): quote regex with brace-quantifier so nginx parses it

Других код-коммитов нет — остальные изменения операционные (на VPS) или в memory (вне репо).
...
2. **Применить nginx-фикс HTML cache-control** при настройке stage-vhost'а: `add_header Cache-Control "no-cache" always;` на server-уровне в основном HTTPS-блоке (см. описание гэпа в «Что сделано»).
...
6. **Push коммитов в `origin/main`** — придержан до завершения staging-настройки. 18 коммитов впереди origin.
