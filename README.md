...
- Контактная форма: вёрстка по spec §5.1, клиентская валидация, honeypot, состояния кнопки, чекбокс согласия.

## Чего в прототипе нет (осознанно)

- **Серверный `/api/contact`** — сейчас форма отвечает локальным mock'ом с задержкой. Подключение nodemailer + `@astrojs/node` + SMTP — следующий шаг.
- **152-ФЗ-обязательное** — текст `/privacy`, реестр обработки, уведомления Роскомнадзора. Это операционная задача, не код.
- **Эксплуатация** — nginx, systemd, certbot, logrotate, CI/CD. Описаны в [`docs/spec/runbook.md`](./docs/spec/runbook.md).
- **Аналитика** — Яндекс Метрика, cookie-баннер, целевые события.
