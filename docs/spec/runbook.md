...
- `Content-Security-Policy` (полный текст — см. §11)
...
**Процедура.** Сверить реально полученные сообщения в Telegram автора и письма на контактный email с ожидаемым объёмом (исходя из объёма обращений в предыдущие месяцы и активности на сайте). Аналитика на сайте не подключена (см. [`decisions.md`](./decisions.md) §5.4), поэтому базовая верификация — вручную пройти deeplink-сценарий формы (`/contact` → заполнить → клик → проверить, что Telegram открыл черновик) при подозрении на регрессию. Триггер активации backend-доставки — §13.5 в [`spec.md`](./spec.md).
...
Внешние домены в политике — только Google Fonts (`fonts.googleapis.com` в `style-src` и `fonts.gstatic.com` в `font-src`). Аналитика, Метрика и иные счётчики не подключены (см. [`decisions.md`](./decisions.md) §5.4) — соответствующие домены в CSP отсутствуют.
